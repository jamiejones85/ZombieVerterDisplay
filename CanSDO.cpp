...
void CanSDO::Setup() {
  ...
  // Initialize the SDO queue
  sdoQueue.Setup();
}

CanSDOQueue& CanSDO::GetQueue() {
  return sdoQueue;
}
...
CanSDO::SetResult CanSDO::SetValue(int id, double value) {
  ...
  // Delegate to the SDO queue (blocking mode for backward compatibility)
  int result = sdoQueue.WriteValueBlocking(id, value);
...
double CanSDO::GetValue(int id) {
  ...
  // Delegate to the SDO queue (blocking mode for backward compatibility)
  int32_t result = sdoQueue.ReadValueBlocking(id);
  return ((double)result) / 32;
}

void CanSDO::Loop() {
  // Process SDO queue when idle
  if (state == IDLE) {
    sdoQueue.Loop();
    return;
  }

  if (state == ERROR || state == OBTAINVALUE) {
    return;
  }
...
void CanSDO::StartJsonFetch() {
  ...
  // Clear any pending SDO requests
  sdoQueue.ClearQueue();
